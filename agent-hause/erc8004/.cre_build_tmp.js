...
var configSchema = exports_external.object({
  ...
  feedOracleUrl: exports_external.string().optional(),
  feedOracleApiKey: exports_external.string().optional(),
  feedOracleSymbols: exports_external.array(exports_external.string()).optional(),
  feedOracleMaxStalenessSeconds: exports_external.number().optional(),
  dryRun: exports_external.boolean().optional(),
  riskThresholds: exports_external.object({
    reserveRatioMin: exports_external.number().optional(),
    pegDeviationMax: exports_external.number().optional(),
    scoreMin: exports_external.number().optional(),
    healthScoreMin: exports_external.number().optional()
  }).optional(),
...
var FEEDORACLE_STABLECOIN_URL = "https://api.feedoracle.io/api/v1/feeds/stablecoin";
var normalizeNumber = (value2) => {
  if (value2 === null || value2 === undefined)
    return null;
  if (typeof value2 === "number" && Number.isFinite(value2))
    return value2;
  if (typeof value2 === "string") {
    const cleaned = value2.trim().replace(/%$/, "");
    const parsed = Number(cleaned);
    return Number.isFinite(parsed) ? parsed : null;
  }
  return null;
};
var numberFromUnknown = exports_external.preprocess((value2) => normalizeNumber(value2), exports_external.number().nullable());
var feedOracleAssetSchema = exports_external.object({
...
var feedOracleResponseSchema = exports_external.object({
  data: exports_external.array(feedOracleAssetSchema)
}).passthrough();
var groqDecisionSchema = exports_external.object({
  riskLevel: exports_external.enum(["low", "medium", "high"]),
  allowUpdate: exports_external.boolean(),
...
var takeFirst = (values) => values[0];
var buildFeedOracleRisk = (asset) => {
  const raw = JSON.parse(JSON.stringify(asset));
  return {
    symbol: asset.symbol,
...
var getFeedOracleApiKey = (runtime2) => runtime2.config.feedOracleApiKey || "";
var getGroqApiKey = (runtime2) => runtime2.config.groq?.apiKey || "";
...
var fetchFeedOracleSnapshot = (nodeRuntime, requestJson) => {
  ...
  const filtered = symbols && symbols.length > 0 ? payload.data.filter((asset) => symbols.includes(asset.symbol)) : payload.data;
  const snapshot = {
...
var getFeedOracleSymbols = (runtime2) => {
  const explicit = runtime2.config.feedOracleSymbols ?? [];
  const fromEvms = runtime2.config.evms.map((evm) => evm.symbol).filter(Boolean);
  const merged = [...new Set([...explicit, ...fromEvms])];
  return merged.length > 0 ? merged : undefined;
};
var parseUpdatedAt = (value2) => {
  if (!value2)
    return null;
  if (value2 instanceof Date)
    return value2;
  if (typeof value2 === "number" && Number.isFinite(value2)) {
    const millis = value2 > 1000000000000 ? value2 : value2 * 1000;
...
var evaluateFeedOracleSnapshot = (snapshot, thresholds, maxStalenessSeconds) => {
  const flags = [];
  ...
  for (const asset of snapshot.assets) {
    ...
    if (maxStalenessSeconds !== undefined) {
      const updatedAt = parseUpdatedAt(asset.raw["updatedAt"]) ?? parseUpdatedAt(asset.raw["lastUpdated"]) ?? parseUpdatedAt(asset.raw["timestamp"]);
  ...
  const summary = allowUpdate ? "FeedOracle snapshot within thresholds." : `FeedOracle snapshot failed thresholds: ${flags.join("; ")}`;
  return { allowUpdate, flags, summary };
};
var createGroqPrompt = (context) => {
  return [
...
var extractJsonFromText = (content) => {
...
var fetchGroqDecision = (nodeRuntime, request) => {
  const httpClient = new ClientCapability2;
...
var fetchMathResult = (nodeRuntime) => {
  const httpClient = new ClientCapability2;
  const req = { url: nodeRuntime.config.apiUrl, method: "GET" };
  const resp = httpClient.sendRequest(nodeRuntime, req).result();
  const bodyText = new TextDecoder().decode(resp.body);
  return BigInt(bodyText.trim());
};
var getFeedOracleSnapshot = (runtime2) => {
  let apiKey = getFeedOracleApiKey(runtime2);
  if (!apiKey) {
    runtime2.log("FeedOracle API key not configured, skipping FeedOracle snapshot.");
    return null;
  }
  const symbols = getFeedOracleSymbols(runtime2) ?? [];
  const request = {
    url: runtime2.config.feedOracleUrl ?? FEEDORACLE_STABLECOIN_URL,
    apiKey,
    symbols
  };
  const requestJson = JSON.stringify(request);
  try {
    const responseJson = runtime2.runInNodeMode(fetchFeedOracleSnapshot, takeFirst)(requestJson).result();
    return JSON.parse(responseJson);
  } catch (error) {
    runtime2.log(`FeedOracle snapshot failed: ${error.message}`);
    return null;
  }
};
var getGroqDecision = (runtime2, context) => {
  let apiKey = getGroqApiKey(runtime2);
  if (!apiKey) {
    runtime2.log("Groq API key not configured, skipping Groq analysis.");
    return null;
  }
  const groqConfig = runtime2.config.groq ?? {};
  const request = {
  ...
  try {
    return runtime2.runInNodeMode(fetchGroqDecision, takeFirst)(request).result();
  } catch (error) {
    runtime2.log(`Groq decision failed: ${error.message}`);
    return null;
  }
};
var doPOR = (runtime2) => {
  runtime2.log(`fetching por url ${runtime2.config.url}`);
  if (runtime2.config.apiUrl) {
    runtime2.log("fetching random number via runInNodeMode");
    const randomVal = runtime2.runInNodeMode(fetchMathResult, consensusMedianAggregation())().result();
    runtime2.log(`aggregated random value: ${randomVal}`);
  }
  ...
  const feedOracleSnapshot = getFeedOracleSnapshot(runtime2);
  if (feedOracleSnapshot) {
    runtime2.log(`FeedOracleSnapshot ${safeJsonStringify(feedOracleSnapshot)}`);
  }
  ...
  const decision = getGroqDecision(runtime2, {
    onChain: {
  ...
  if (feedOracleSnapshot && runtime2.config.riskThresholds) {
    const evaluation = evaluateFeedOracleSnapshot(feedOracleSnapshot, runtime2.config.riskThresholds, runtime2.config.feedOracleMaxStalenessSeconds);
    runtime2.log(`RiskThresholds ${safeJsonStringify(evaluation)}`);
    if (!evaluation.allowUpdate) {
      runtime2.log("Threshold gate blocked updateReserves; exiting without on-chain update.");
      return reserveInfo.totalReserve.toString();
    }
  }
  if (decision) {
    runtime2.log(`GroqDecision ${safeJsonStringify(decision)}`);
    if (!decision.allowUpdate) {
      runtime2.log("Groq decision blocked updateReserves; exiting without on-chain update.");
      return reserveInfo.totalReserve.toString();
    }
  }
  if (runtime2.config.dryRun) {
    runtime2.log("Dry-run enabled; skipping updateReserves.");
    return reserveInfo.totalReserve.toString();
  }
